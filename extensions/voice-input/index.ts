...
function getConfig(): {
  ...
  const whisperUrl =
    ...
    envVars.WHISPER_URL ||
    "https://whisper.local.johnyan.net";
  ...
  const silenceDuration = parseInt(
    process.env.SILENCE_DURATION || envVars.SILENCE_DURATION || "2",
    10,
  );
  ...
  return {
    ...
    micDevice,
    micSource,
    silenceDuration: isNaN(silenceDuration) ? 2 : Math.max(1, silenceDuration),
  };
}
...
async function pcmToWav(pcmBuffer: Buffer, outputPath: string): Promise<void> {
  ...
  offset += 4;
  header.writeInt32LE(bufferSize, offset);
  offset += 4;
  ...
  offset += 4;
  header.writeInt32LE(16, offset);
  offset += 4; // chunk size
  header.writeInt16LE(1, offset);
  offset += 2; // PCM
  header.writeInt16LE(numChannels, offset);
  offset += 2;
  header.writeInt32LE(sampleRate, offset);
  offset += 4;
  header.writeInt32LE(byteRate, offset);
  offset += 4;
  header.writeInt16LE(blockAlign, offset);
  offset += 2;
  header.writeInt16LE(bitsPerSample, offset);
  offset += 2;

  // data chunk
  header.write("data", offset);
  offset += 4;
  header.writeInt32LE(dataSize, offset);
  offset += 4;
...
export function recordAudio(
  ...
  const promise = new Promise<{ filePath: string; silenceEnd?: number }>(
    (resolve, reject) => {
      ...
      let silentWindows = 0;
      let totalBytes = 0; // received PCM bytes (for the silence-end timestamp)
      ...
      const SAMPLE_RATE = 16000; // matches ffmpeg -ar 16000, s16le mono = 2 bytes/sample
      ...
      const settle = async (silenceEnd?: number) => {
        ...
        if (proc.exitCode === null) {
          try {
            ...
            console.error("[voice-input] failed to stop ffmpeg:", err);
          }
        }
      ...
      proc.stdout.on("data", (chunk: Buffer) => {
        ...
        currentWindow =
          remaining > 0
            ? Buffer.from(remainder.subarray(offset))
            : Buffer.alloc(0);
      });
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerShortcut("alt+q", {
    ...
    handler: async (ctx) => {
      ...
      if (busy) {
        ctx.ui.notify(
          "Voice: busy — current recording/transcription still running",
          "warning",
        );
        return;
      }
      ...
      void (async () => {
        try {
          ...
          ctx.ui.notify(
            `🎙 Listening... (press Alt+Q to stop, or ${config.silenceDuration}s silence)`,
            "info",
          );
          ...
          if (text.trim()) {
            ...
            pi.sendUserMessage(voiceTextToInput(text), {
  ...
  pi.on("input", async (event, ctx) => {
    ...
    if (/^\/voice\s+mic\s*$/i.test(event.text)) {
      ctx.ui.notify(
        `Voice mic: ${config.micDevice} — ${MIC_SOURCE_LABELS[config.micSource]}`,
        "info",
      );
